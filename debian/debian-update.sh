...
then
	...
	NEWYORKCOIN_VER="$NEWYORKCOIN_VER_NO_BIT-x86_64-linux-gnu"
	else
	NEWYORKCOIN_DL_URL=$NEWYORKCOIN_DL_URL_32
	NEWYORKCOIN_VER="$NEWYORKCOIN_VER_NO_BIT-i686-pc-linux-gnu"
	fi
	...
	then
		...
		#Add $DEBIAN_WEBSITE_DIR to the new newyorkcoin-node-status.py script
		echo "Add the distributions website dir to the newyorkcoin-nodes-status.py script"
		sed -i -e '13iff = open('"'$DEBIAN_WEBSITE_DIR/index.html'"', '"'w'"')\' $HOME/scripts/newyorkcoin-node-status.py
