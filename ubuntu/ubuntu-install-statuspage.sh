...
echo "*/10 * * * * /usr/bin/python $HOME/scripts/newyorkcoin-node-status.py" >> /$HOME/scripts/crontempfile
crontab $HOME/scripts/crontempfile
rm $HOME/scripts/crontempfile

#Add $UBUNTU_WEBSITE_DIR to the newyorkcoin-node-status.py script
echo "Add the distributions website dir to the newyorkcoin-node-status.py script"
sed -i -e '13iff = open('"'$UBUNTU_WEBSITE_DIR/index.html'"', '"'w'"')\' $HOME/scripts/newyorkcoin-node-status.py

#Add NewYorkCoin rpc user and password to the newyorkcoin-node-status.py script
echo "Add NewYorkCoin rpc user and password to the newyorkcoin-node-status.py script"
